...
s32 dcreadaifccodebook(FILE *fhandle, s32 ****table, s16 *order, s16 *npredictors)
{
...
ALADPCMloop *dreadlooppoints(FILE *ifile, s16 *nloops)
{
...
s32 dinner_product(s32 length, s32 *v1, s32 *v2)
{
    s32 out = 0;
...
void my_decodeframe(u8 *frame, s32 *state, s32 order, s32 ***coefTable)
{
    ...
    for (s32 j = 0; j < 2; j++) {
        ...
        for (s32 i = 0; i < 8; i++) {
            ...
            state[ind] = dinner_product(order + i, coefTable[optimalp][i], in_vec) + ix[ind];
        }
    }
}
...
void my_encodeframe(u8 *out, s16 *inBuffer, s32 *state, s32 ***coefTable, s32 order, s32 npredictors)
{
    ...
    for (s32 k = 0; k < npredictors; k++) {
        for (s32 j = 0; j < 2; j++) {
            ...
            for (s32 i = 0; i < 8; i++) {
                prediction[j * 8 + i] = dinner_product(order + i, coefTable[k][i], inVector);
    ...
    for (s32 j = 0; j < 2; j++) {
        ...
        for (s32 i = 0; i < 8; i++) {
            prediction[j * 8 + i] = dinner_product(order + i, coefTable[optimalp][i], inVector);
            e[j * 8 + i] = inVector[i + order] = inBuffer[j * 8 + i] - prediction[j * 8 + i];
        }
    }
    ...
    for (s32 nIter = 0, again = 1; nIter < 2 && again; nIter++) {
        ...
        for (s32 j = 0; j < 2; j++) {
            ...
            for (s32 i = 0; i < 8; i++) {
                prediction[base + i] = dinner_product(order + i, coefTable[optimalp][i], inVector);
                s32 se = inBuffer[base + i] - prediction[base + i];
                ix[base + i] = dqsample(se, scale);
...
int aifc_decode(char* in, char* out) {
    s16 order = -1;
    s16 nloops = 0;
    ...
    if ((ifile = fopen(in, "rb")) == NULL) {
        fail_parse("AIFF-C file could not be opened");
        exit(1);
    }

    if ((ofile = fopen(out, "wb")) == NULL) {
        fprintf(stderr, "aifc_decode: output file could not be opened [%s]\n", out);
        exit(1);
    }
            ...
            if (ts == 0x73746f63) { // stoc
                ...
                if (len == 11) {
                    ...
                    if (strcmp("VADPCMCODES", ChunkName) == 0) {
                        ...
                        dcreadaifccodebook(ifile, &coefTable, &order, &npredictors);
                    }
                        ...
                        aloops = dreadlooppoints(ifile, &nloops);
                        if (nloops != 1) {
