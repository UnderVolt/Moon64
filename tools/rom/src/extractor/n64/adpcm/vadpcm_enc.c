...
int vadpcm_enc(char* table, char* aiff, char* aifc)
{
    s32 c;
    s16 nloops = 0;
    s16 numMarkers;
    ...
    FILE *ifile;
    FILE *ofile;
    char* progname = "vadpcm";
    if ((fhandle = fopen(table, "r")) == NULL) {
    ...
    ifile = fopen(aiff, MODE_READ);
    ofile = fopen(aifc, MODE_WRITE);
